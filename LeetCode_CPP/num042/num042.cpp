    ...
    int trap(vector<int>& height) {
        ...
        for(k = j+1; k < length; ++k){
            ...
            if(flag && height[k] > height[k-1]){    //lowest point
                ...
                result += lower * (k-1-index) - realBlock;    //(k-1-j-1)
                flag = true;
                block.clear();
                --k;
                j = k;
        ...
        cout << "flag: " << flag << ", j: " << j << ", k-1: " << k-1 << ", result: " << result << endl;
        cout << "------------------" << endl;
        if(!flag){
            int lower = min(height[j], height[k-1]);
            int size = block.size();
...
int main(void){
	Solution sol;
	vector<int> height;
	height.push_back(5);height.push_back(4);height.push_back(1);height.push_back(2);
